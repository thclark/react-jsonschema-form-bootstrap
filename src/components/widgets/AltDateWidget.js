...
class AltDateWidget extends Component {
  ...
  renderElement = (elemProps, index) => {
    const { id, disabled, readonly, autofocus, onBlur, registry } = this.props;
    return (
      <DateElement
        rootId={id}
  ...
  render() {
    return (
      <ul className="list-inline">
        {this.dateElementProps.map((elemProps, index) => (
          <li key={index} className="list-inline-item">
            {this.renderElement(elemProps, index)}
          </li>
        ))}
        <li className="list-inline-item">
          <button className="btn btn-info btn-now" onClick={this.setNow}>
            Now
          </button>
        </li>
        <li className="list-inline-item">
